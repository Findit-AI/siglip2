...
//! (e.g. an `mlx-community` mirror). The loader reads `config.json`, then
...
//! `model.safetensors`, a `*.gguf`, or a `*.npz` — runs the model's `sanitize`
//! key-remap, and builds the dual tower via `mlxrs`.
//!
...
//! directly and read the sibling `config.json` from the file's parent directory,
//! for callers who already know the format and location.
//!
...
/// The directory the sibling `config.json` is read from when an explicit-format
/// constructor is handed a **weight file path**: the file's parent directory, or
/// the current directory (`.`) when `weights` is a bare filename with no parent
/// component (so `from_safetensors("model.safetensors")` reads `./config.json`,
/// not a config at the filesystem root).
...
impl MlxModel {
  ...
  /// Load a model from an **exact** `*.npz` file path. The `config.json` is read
  /// from the weight file's parent directory (see [`weights_parent`]).
  #[cfg(feature = "npz")]
  ...
  /// Load a model from an **exact** `*.gguf` file path. The `config.json` is read
  /// from the weight file's parent directory (see [`weights_parent`]); the gguf's
  ...
  /// pick the dense-vs-quantized entry point, and build the SigLIP2 NaFlex dual
  /// tower.
  ///
  ...
  /// config read + full [`Siglip2NaflexConfig::validate`] run BEFORE `load`, so a
  /// malformed config fails fast and never touches the weight file.
  fn construct(
    dir: &Path,
    load: impl FnOnce() -> Result<std::collections::HashMap<String, mlxrs::Array>>,
  ) -> Result<Self> {
    ...
    // Run the FULL `Siglip2NaflexConfig::validate` (it pins both towers'
    // `model_type`, pins vision `num_channels` to 3, and requires every dimension
    // / count — `patch_size`, the resolved `max_num_patches`, the text
    // `max_position_embeddings`, the layer / head counts, the per-head split —
    // structurally valid) BEFORE any weight is loaded, so a malformed config
    // fails fast with a typed error and never touches the (expensive) weight
    // file. No UPPER cap is imposed on `max_num_patches`: the MLX preprocessor is
    // parameterized by the checkpoint's own declared budget (unlike the ONNX
    // export, which bakes a fixed `256`), so the checkpoint author owns it.
    config.validate().map_err(Error::from_mlx)?;

    // Pull the geometry the preprocessing + text padding need off the now-validated
    // config (each field is positive — and `num_channels == 3` — by the validation
    // above) and narrow it to the `u32` / `usize` the preprocessor consumes. The
    // `try_from` is the type narrowing; the positivity / RGB `filter`s are a kept
    // safety net (validate already guarantees them).
...
mod tests {
  ...
  /// A `config.json` that PARSES but fails the full
  /// [`Siglip2NaflexConfig::validate`] on a NON-geometry field — here a wrong
  /// top-level `model_type` (validate pins it to `"siglip2"`) — is rejected with
  /// a typed [`Error::Mlx`] BEFORE any weight is loaded. [`from_config_json`]
  /// writes ONLY the malformed `config.json` (no `model.safetensors`), so a
  /// `from_dir` that nonetheless errors proves the full config validation runs
  /// ahead of the (expensive) weight load, not after it in `from_weights`.
  #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
  #[test]
  fn invalid_config_is_rejected_before_weight_load() {
    // Valid JSON with present (defaulted-geometry) tower objects — so it PARSES
    // — but a top-level `model_type` the validator rejects. The two tower configs
    // are required keys (no serde default), so an empty `{}` for each supplies
    // them with their defaulted, valid geometry; the failure is then on a field
    // OTHER than the geometry dims, and no weight file is written in the dir.
    let json = r#"{"model_type": "not_siglip2", "text_config": {}, "vision_config": {}}"#;
    let err = from_config_json("model_type", json)
      .err()
